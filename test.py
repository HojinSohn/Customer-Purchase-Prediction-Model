...
def __order_and_cast(df_test, customers_features, products_features, pairs_features, all_types) -> pd.DataFrame:
    # Order the columns in the dataframe to match the order of the data used to train the model
    order_df = ['time_date', 'customer_id', 'product_id'] + list(customers_features.columns) + list(products_features.columns) + list(pairs_features.columns)
...
def __get_top_k(target_entity_table, probabilities, df, src_entity, tar_entity):
    ...
    while idx < len(probabilities):
        ...
        indexes = np.argsort(prob_chunk)[:-13:-1]
...
def test():
    ...
    new_features = du.feature_engineering(BASEPATH, src_entity, tar_entity, source_entity_table, target_entity_table)
    ...
    while tot_rows > 0:
        ...
        df_test, all_types, customers_features_batch, products_features_batch, pairs_features_batch = du.pair_feature_engineering(df_test, all_types, customers_features, products_features)
        
        
        # Force correct columns order and cast the right dtypes
        df_test = __order_and_cast(df_test, customers_features_batch, products_features_batch, pairs_features_batch, all_types)
# maybe not necessary?
